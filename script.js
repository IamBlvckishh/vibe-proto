...
// Attach explicit listener to close button
...
function vote(id, type, e) {
    ...
    if (type === 'up') p.score += impact;
    else p.score -= Math.floor(impact / 2);
...
document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.onclick = () => {
        ...
        if(target === 'arenaTab') {
            loadArena();
            window.scrollTo(0, 0); // Reset scroll to top to show header
        }
